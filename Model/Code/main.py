...
parser.add_argument('--prior_edges', type=str, default="/content/drive/MyDrive/MultiCausGRN/PBMC_multiome_preprocessed/PBMC/Model_INPUT_A/Positive_edges.csv",
                    help="CSV with columns tf_idx,target_idx defining causal/prior positive edges")
...
torch.use_deterministic_algorithms(False)  # True can crash with some ops
...
# Train loader stays TRAIN ONLY (for batching)
# Train loader stays TRAIN ONLY (for batching)
train_load = scRNADataset(train_data, feature.shape[0], flag=args.flag)

# ---- Build adjacency from PRIOR POSITIVE EDGES ONLY (no leakage) ----
prior_path = args.prior_edges
prior_df = pd.read_csv(prior_path)

# expect columns: tf_idx,target_idx (ints)
prior_df = prior_df[["tf_idx", "target_idx"]].drop_duplicates().copy()
prior_df["tf_idx"] = prior_df["tf_idx"].astype(np.int64)
prior_df["target_idx"] = prior_df["target_idx"].astype(np.int64)

# create a fake "train_set" array with label=1 for Adj_Generate
prior_pairs = np.column_stack([
    prior_df["tf_idx"].values,
    prior_df["target_idx"].values,
    np.ones(len(prior_df), dtype=np.int64)
]).astype(np.int64)

print("Prior edges loaded:", prior_pairs.shape[0], "from", prior_path)
print("Prior idx max:", prior_pairs[:, :2].max(), "n_genes:", feature.shape[0])

adj_load = scRNADataset(prior_pairs, feature.shape[0], flag=False)
adj = adj_load.Adj_Generate(tf, direction=True, loop=args.loop)
...
for epoch in range(args.epochs):
    ...
    # score = torch.sigmoid(score)
    print("val y unique:", torch.unique(validation_data[:, -1]).detach().cpu().numpy())
    ...
    print(f"[best so far] epoch={best_epoch} val_AUPR={best_val_aupr:.4f}")
torch.save(best_state, model_path + data_type+' '+str(num)+'_best.pkl')
model.load_state_dict(torch.load(model_path + data_type+' '+str(num)+'_best.pkl'))
model.eval()
...
# Load best weights BEFORE scoring
model.load_state_dict(best_state)
model = model.to(device)
model.eval()
print(f"Loaded best epoch={best_epoch} with val_AUPR={best_val_aupr:.4f}")
...
print("test y unique:", torch.unique(test_data[:, -1]).detach().cpu().numpy())
AUC, AUPR, AUPR_norm = Evaluation(y_true=test_data[:, -1], y_pred=score, flag=args.flag)
