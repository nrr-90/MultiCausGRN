...
# ---- Build adjacency from ALL splits (train+val+test) using POSITIVES only ----
all_pairs = np.vstack([train_data, validation_data, test_data]).copy()

# make sure indices are int (Adj_Generate reads pos[0], pos[1])
all_pairs[:, 0] = all_pairs[:, 0].astype(np.int64)
all_pairs[:, 1] = all_pairs[:, 1].astype(np.int64)
all_pairs[:, 2] = all_pairs[:, 2].astype(np.float32)

adj_load = scRNADataset(all_pairs, feature.shape[0], flag=args.flag)

# direction=True is better for TF->Target (keeps the TF direction)
